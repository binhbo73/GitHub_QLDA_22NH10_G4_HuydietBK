...
export function ChatMessage({ message, isBot = false, timestamp, onEdit }: ChatMessageProps) {
  ...
  return (
    <div className={`flex ${isBot ? "justify-start" : "justify-end"} mb-4 group`}>
      <div className={`max-w-[80%] ${isBot ? "order-2" : "order-1"} relative`}>
        <div className={`rounded-2xl px-4 py-2 ${isBot ? "bg-purple-900/30 text-white" : "bg-purple-600 text-white"} ${isEditing ? "min-w-[500px]" : ""}`}>
          {isEditing && isBot ? (
