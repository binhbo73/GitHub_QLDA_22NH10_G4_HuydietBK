...
export function ChatSidebar({ onNewChat }: { onNewChat?: () => void }) {
    ...
    return (
        <div className="w-64 bg-gray-900 border-r border-purple-900 flex flex-col min-h-full">
            ...
            </div>            <ScrollArea className="flex-1 px-2 overflow-y-auto">
                <div className="space-y-2 pr-2">
                    {chatHistory
                        ...
                        .map((chat) => (
                            <div
                                key={chat.id}
                                className={`group flex items-center gap-2 rounded-lg px-3 py-2 hover:bg-purple-900/20 transition-colors ${pathname.includes(chat.id) ? 'bg-purple-900/30' : ''
                                    }`}
                            >
